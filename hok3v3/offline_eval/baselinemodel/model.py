...
class Model(nn.Module):
    def __init__(self, Config):
        ...
        self.all_hero_feature_dim = (
            int(np.sum(self.config.DIM_OF_HERO_FRD)) + int(np.sum(self.config.DIM_OF_HERO_EMY)) + int(np.sum(self.config.DIM_OF_HERO_MAIN))
        )
        self.all_soldier_feature_dim = int(np.sum(self.config.DIM_OF_SOLDIER_1_10)) + int(np.sum(self.config.DIM_OF_SOLDIER_11_20))
        self.all_organ_feature_dim = int(np.sum(self.config.DIM_OF_ORGAN_1_3)) + int(np.sum(self.config.DIM_OF_ORGAN_4_6))
    ...
    def forward(self, data_list):
        ...
        for hero_index, hero_data in enumerate(data_list):
            ...
            # print(hero_data,hero_data.shape)
            ...
            hero_vec_list = split_feature_vec[0].split(
                [int(np.sum(self.config.DIM_OF_HERO_FRD)), int(np.sum(self.config.DIM_OF_HERO_EMY)), int(np.sum(self.config.DIM_OF_HERO_MAIN))], dim=1
            )
            ...
            soldier_vec_list = split_feature_vec[1].split(
                [int(np.sum(self.config.DIM_OF_SOLDIER_1_10)), int(np.sum(self.config.DIM_OF_SOLDIER_11_20))], dim=1
            )
            soldier_1_10 = soldier_vec_list[0].split(self.config.DIM_OF_SOLDIER_1_10, dim=1)
            _soldier_1_10 = torch.stack(soldier_1_10, dim=1)
            soldier_11_20 = soldier_vec_list[0].split(self.config.DIM_OF_SOLDIER_11_20, dim=1)
            _soldier_11_20 = torch.stack(soldier_11_20, dim=1)

            organ_vec_list = split_feature_vec[2].split([int(np.sum(self.config.DIM_OF_ORGAN_1_3)), int(np.sum(self.config.DIM_OF_ORGAN_4_6))], dim=1)
            ...
            monster_1_20 = monster_vec_list.split(self.config.DIM_OF_MONSTER_1_20, dim=1)
        ...
        self.probs_h0 = torch.flatten(torch.cat(all_hero_fc_result_list[0], 1), start_dim=1)
        self.probs_h1 = torch.flatten(torch.cat(all_hero_fc_result_list[1], 1), start_dim=1)
        self.probs_h2 = torch.flatten(torch.cat(all_hero_fc_result_list[2], 1), start_dim=1)
