...
class NetworkModel(nn.Module):
    ...
    def compute_loss(self, data_list, rst_list):
        ...
        for hero_index, hero_data in enumerate(data_list):
            ...
            for task_index, is_reinforce_task in enumerate(self.hero_is_reinforce_task_list[hero_index]):
                if is_reinforce_task:
                    ...
                    one_hot_actions = nn.functional.one_hot(_hero_action_list[task_index].long(), _hero_label_size_list[task_index])
                    ...
                    label_logits_subtract_max = torch.clamp(
                        ...
                        - torch.max(_hero_fc_label_result[task_index] - legal_action_flag_list_max_mask, dim=1, keepdim=True).values,
                        -boundary,
                        1,
                    )
                    ...
                    label_exp_logits = _hero_legal_action_flag_list[task_index] * torch.exp(label_logits_subtract_max) + self.min_policy
                    ...
                    policy_cost = policy_cost - torch.sum(
                        torch.minimum(surr1, surr2) * (_hero_weight_list[task_index].float()) * _hero_frame_is_train
                    ) / torch.maximum(torch.sum((_hero_weight_list[task_index].float()) * _hero_frame_is_train), torch.tensor(1.0))
            ...
            for task_index, is_reinforce_task in enumerate(self.hero_is_reinforce_task_list[hero_index]):
                if is_reinforce_task:
                    ...
                    temp_entropy_loss = -torch.sum(
                        (temp_entropy_loss * _hero_weight_list[task_index].float() * _hero_frame_is_train)
                    ) / torch.maximum(
        ...
        return total_loss, [
            total_loss,
            [all_hero_loss_list[0][1], all_hero_loss_list[0][2], all_hero_loss_list[0][3]],
    ...
    def format_data(self, datas):
        ...
        for hero_index in range(self.hero_num):
            ...
            sequence_data, lstm_cell_data, lstm_hidden_data = data_list[hero_index].float().split(hero_sequence_data_split_shape, dim=1)
