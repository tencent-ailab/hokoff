        ...
        for hero_index in range(len(each_hero_data_list)):
            with tf.variable_scope("hero_%d" % hero_index, reuse=tf.AUTO_REUSE):
                with tf.xla.experimental.jit_scope(self.use_xla):
                    ...
                    organ_dim = int(np.sum(DimConfig.DIM_OF_ORGAN_1_3)) + int(np.sum(DimConfig.DIM_OF_ORGAN_4_6))
                    ...
                    with tf.variable_scope("hero%d_feature_vec_split" % hero_index):
                        ...
                        soldier_vec_list = tf.split(
                            ...
                            [int(np.sum(DimConfig.DIM_OF_SOLDIER_1_10)), int(np.sum(DimConfig.DIM_OF_SOLDIER_11_20))],
                            axis=1,
                        )
                        organ_vec_list = tf.split(
                            feature_vec_split_list[2], [int(np.sum(DimConfig.DIM_OF_ORGAN_1_3)), int(np.sum(DimConfig.DIM_OF_ORGAN_4_6))], axis=1
                        )
                        ...
                        monster_1_20 = tf.split(monster_vec_list, DimConfig.DIM_OF_MONSTER_1_20, axis=1)
                        organ_1_3 = tf.split(organ_vec_list[0], DimConfig.DIM_OF_ORGAN_1_3, axis=1)
                        organ_4_6 = tf.split(organ_vec_list[1], DimConfig.DIM_OF_ORGAN_4_6, axis=1)
