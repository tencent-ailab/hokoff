...
class CommonFunc:
    ...
    def generate_data(*samples):
        ...
        while start_idx < end_idx:
            ...
            sys.stdout.flush()
            send_sample = struct.pack("<I", int(batch_idx)) + struct.pack("<I", int(1000001)) + struct.pack("<I", int(sample_num))
            str_com = ""
            for frame_idx in range(0, sample_num):
                frame_no = start_idx + frame_idx
                str_com = samples[frame_no]
                send_sample += struct.pack("<I", int(len(str_com))) + struct.pack("<%ss" % (len(str_com)), str_com)
