...
from config.common_config import DimConfig
from config.common_config import ModelConfig as Config
...
class Algorithm:
            ...
            if self.is_reinforce_task_list[task_index]:
                ...
                label_logits_subtract_max = tf.clip_by_value(
                    (
                        ...
                        - tf.reduce_max(
                            fc2_label_list[task_index] - legal_action_flag_list_max_mask,
                            axis=1,
                            keep_dims=True,
                        )
                    ),
                ...
                temp_policy_loss = -tf.reduce_sum(tf.to_float(weight_list[task_index]) * tf.minimum(surr1, surr2)) / tf.maximum(
        ...
        for task_index in range(len(self.is_reinforce_task_list)):
            if self.is_reinforce_task_list[task_index]:
                temp_entropy_loss = -tf.reduce_sum(
                    ...
                    * tf.log(label_probability_list[current_entropy_loss_index] + 0.00001),
                    axis=1,
                )
                temp_entropy_loss = -tf.reduce_sum((temp_entropy_loss * tf.to_float(weight_list[task_index]))) / tf.maximum(
                    tf.reduce_sum(tf.to_float(weight_list[task_index])), 1.0
    ...
    def _inference(self, seri_vec, init_lstm_cell, init_lstm_hidden, only_inference=False):
        ...
        hero_dim = int(np.sum(DimConfig.DIM_OF_HERO_FRD)) + int(np.sum(DimConfig.DIM_OF_HERO_EMY)) + int(np.sum(DimConfig.DIM_OF_HERO_MAIN))
        ...
        with tf.variable_scope("fc2_label_%d" % (len(self.label_size_list) - 1)):
            ...
            ulti_tar_embedding = tf.layers.dense(tar_embedding, self.target_embed_dim, use_bias=False)  # (Batch_size, unit_num,embed)
        ...
        with tf.variable_scope("fc1_value"):
            ...
            fc1_value_result = tf.nn.relu(
                (tf.matmul(reshape_lstm_outputs_result, fc1_value_weight) + fc1_value_bias),
